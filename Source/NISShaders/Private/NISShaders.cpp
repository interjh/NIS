...
#else
			// WITH_HD Begin
			if(DoesPlatformSupportDXC(Parameters.Platform) && (Parameters.Platform == SP_METAL_SM5 || Parameters.Platform == SP_METAL_SM6))
			{
				return true;
			}
			// WITH_HD End
...
)
{
	...
	if (bHalfPrecisionMode == -1)
	{
		...
		// WITH_HD Begin
#elif PLATFORM_MAC
		static const bool bIsDx12 = FCString::Strcmp(GDynamicRHI->GetName(), TEXT("Metal")) == 0;
		// WITH_HD End
#else
